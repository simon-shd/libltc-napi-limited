const addon = require('../build/Release/node-libltc-limited-native');
...
class LibltcNapiLimited extends Transform {
    ...
    _transform(chunk, encoding, callback) {
        ...
        try {
            ...
            let return_opt = this.decodeChunk(chunk, tc => {
                ...
                } else if (this._libltcData.previousFrame === frameNum) {
                    console.log("repeated frame: " + frameNum);
                } else {
                ...
                if (this._libltcData.jumpFrames !== 0 && this._libltcData.determinedFps) {
                    // console.log("o: "+tc);
                    tc = jumpForward(tc, this._libltcData.jumpFrames, this._libltcData.fps);
                }
                this.push(tc);
            });
    ...
    _destroy(err, callback) {
        try {
            this._addonInstance.freeDecoder();
        } catch (e) {
            callback(e || err);
        }
    }
